...
def _render_index(safe_report: dict[str, Any], unsafe_report: dict[str, Any]) -> str:
    safe_card = _report_card("Baseline: readable periodic control", safe_report, "safe-report.html")
    unsafe_card = _report_card("Same program + one risky line", unsafe_report, "unsafe-report.html")
...
<head>
  ...
  <style>
    ...
    .card, .change {{ border: 1px solid currentColor; border-radius: 18px; padding: 22px; }}
    ...
    pre {{ overflow-x: auto; padding: 14px; border: 1px solid currentColor; border-radius: 12px; }}
    .change, .steps {{ margin-top: 24px; }}
    li {{ margin: .55rem 0; }}
...
<body>
  <header>
    ...
    <p class="lead">These programs are byte-for-byte identical except for one added line inside the periodic control path. Saga analyzes both through the same file-based path used by <code>saga-control-report</code>.</p>
  </header>
  <main>
    <div class="grid">{safe_card}{unsafe_card}</div>
    <section class="change">
      <div class="eyebrow">The only source change</div>
      <h2>One host-time call is added inside the tick</h2>
      <pre><code>+ {risky_line}</code></pre>
      <p>Frequency, budget, helper function, return expression, and output stay unchanged. Removing this one line reconstructs the safe source exactly.</p>
    </section>
    <section class="steps">
      ...
      <ol>
        ...
        <li>The unsafe example is not a prerecorded failure: the normal file-based analyzer emits the diagnostic and source location.</li>
